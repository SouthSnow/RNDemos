...
var WKListView = React.createClass({
	...
	getData: function(url) {
		...
		.then(json => {
			this.setState({
				...
				isLoading: false,
				// sliderImages: this.state.sliderImages.push(json.result['list']),
			}),
			sliderImages = json.result['list'];
			console.log('请求成功' + sliderImages.length);
	...
	renderRow: function(rowData: Object, sectionID, rowID) {
		...
		return (
			<View style = {{height: 230, flex: 1}}>
				<TouchableHighlight onPress={()=>this.pushNextVC(rowID)}>
	...
	pushNextVC: function(rowID: number) {

		var pic_shows = sliderImages[rowID].pic_show;
		console.log('pic_show' + ' : ' + pic_shows);

		this.props.navigator.push({
			title:this.props.title,
			component: Detail,
			passProps: {
                sliderImgs:pic_shows,
              },
		});
	},
